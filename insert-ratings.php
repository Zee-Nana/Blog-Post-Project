<?php

    require "config.php";

    //'insert' is generated from jquery ajax
    if(isset($_POST['insert'])) {

        $post_id = $_POST['post_id'];
        $rating = $_POST['rating'];
        $user_id = $_POST['user_id'];

        
        //update rating number when user changes rates number using post_id and user_id 
        $delete = $conn->prepare("DELETE FROM rates WHERE post_id = '$post_id' AND user_id='$user_id'");
        $delete->execute();

        //insert values into rates mysql database
        $insert = $conn->prepare("INSERT INTO rates(post_id, rating, user_id)
        VALUES(:post_id, :rating, :user_id)");

            $insert->execute([
                ':post_id' => $post_id,
                ':rating' => $rating,
                ':user_id' => $user_id,
            ]);


    }



?>